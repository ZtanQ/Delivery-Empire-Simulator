...
public class MGR_Inventory : Manager<MGR_Inventory>
{
    ...
    private Dictionary<DATA_ProductSO, int> stock = new Dictionary<DATA_ProductSO, int>(); 
    private Dictionary<string, int> shelfContents = new Dictionary<string, int>();
    ...
    public int GetStock(DATA_ProductSO product)
    {
        ...
        return stock.TryGetValue(product, out int quantity) ? quantity : 0;
    }
    ...
    public void DebugAddStock(DATA_ProductSO product, int amount)
    {
        ...
        Debug.Log($"Inventory: {product.DisplayName} ={newQuantity}");       
    }
